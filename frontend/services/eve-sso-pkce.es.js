...
const __reflectGet = Reflect.get
const __reflectSet = Reflect.set
...
const pad_string_1 = padString$1
...
const base64url = base64url$3.exports
...
class EveSSOAuth {
  ...
  async refreshToken (refreshToken, scopes) {
    try {
      ...
      form.append('client_id', this.config.clientId)
      if (scopes !== void 0) { form.append('scope', scopes.join(' ')) }
      const url = new URL(TOKEN_PATH, BASE_URI).toString()
      const response = await this._fetchToken(url, {
        method: 'POST',
        headers: {
